...
if ( $npr != 2 ){
  for $i (0..$#G){
    if ($F[$i] =~ /txt/){
      $str = "./bin/qseq2fastq.pl -clean=1 -in=".$F[$i]." > ".$G[$i];
      print $str."\n";
      (system($str) == 0 or die "Could not execute ".$str."\n") if ($test == 0);
    }
  ...
  if ( $nsam == 0 ){
    if ( $cel ){
      $str = "bwa samse -n ".$n." ".$r." ".join(" ",($H[1], $G[1]))." > ".$out.".sam";
    }else{
      if ($pflag){
...
if ( $npr == 0 || $npr == 2){
    $s = 1;
    $s = 0 if $pflag;
    $str = "./bin/process_sam_strand.pl -in=".$out.".sam -s=".$s." -edit=".$edit;
    if ( $cel ){
      $str = "./bin/process_sam_cel_seq_np.pl -in=".$out.".sam -bfq=".$G[0]." -bc=".$bar." -anno=".$anno." -rb=".$rb." -rb_len=".$rb_len." -edit=".$edit;
      if ( $fstr ){
	$str = $str." -fstr=".$fstr;
      }
      if ( $dprm ){
